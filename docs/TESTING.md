...
- **백엔드 응답 규격:** [API-RESPONSES.md](./API-RESPONSES.md) (프론트가 백엔드에게 받는 정보)
...
프로젝트 루트에 `.env` 파일을 만들고, 백엔드 주소를 넣습니다.
...
- CORS가 켜져 있어야 합니다 (프론트 localhost:3000에서 API 호출 가능해야 함).
...
1. **카카오 로그인 후 JSON 응답 대신 프론트로 리다이렉트**
   - 리다이렉트 URL: `http://localhost:3000/login/callback?access_token=...&refresh_token=...&user_id=...`
   - 백엔드 Cursor에 전달한 “리다이렉트하라”는 수정이 적용되어 있어야 함.

2. **카카오 앱 설정 (개발자 콘솔)**
   - Redirect URI에 백엔드 콜백 주소가 등록되어 있어야 함 (예: `http://localhost:8000/callback`).
...
2. **카카오로 로그인** 버튼 클릭.
3. 카카오 로그인 화면에서 로그인.
4. 로그인 성공 시 **대시보드**(http://localhost:3000/dashboard)로 자동 이동하는지 확인.

- JSON만 보이고 리다이렉트가 안 되면 → 백엔드가 아직 리다이렉트를 안 하고 있는 것이므로, 백엔드 수정이 필요합니다.

### "콜백 페이지에서 계속 로딩만 돼요"

프론트는 `GET .../request-access-token-after-redirection?code=xxx` 호출 시 **`Accept: application/json`** 헤더를 보냅니다.

- **권장(백엔드):** 이 요청에는 **302가 아니라 200 + JSON**으로 응답해 주세요.  
  - 응답 본문: `{ "access_token": "...", "refresh_token": "...", "user": { "id", "nickname", "email", "profile_image_url" } }`  
- 그러면 프론트가 토큰을 저장하고 바로 대시보드로 넘깁니다.
- 302만 주면 브라우저가 리다이렉트를 따라가서 프론트가 302를 처리하지 못하고 로딩만 계속될 수 있습니다.
...
| 3 | `npm run dev`로 프론트 실행 | http://localhost:3000 접속 |
